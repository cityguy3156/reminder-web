...
// ===== BLE STATE =====
    ...
    const applyMobileHomeLayout = () => {
      ...
      if (mobilePortrait) {
        ...
        for (const [btn, col] of landscapeBtns) {
          if (!btn) continue;
          btn.style.gridColumn = col;
          btn.style.gridRow = "2";
          btn.style.position = "relative";
    ...
    // Image speed (ms per image) slider
    this.speedRow = document.createElement("div");
    Object.assign(this.speedRow.style, {
      display: "flex",
      alignItems: "center",
      justifyContent: "center",
      gap: "12px",
    ...
    this.speedLabel.textContent = "Image Speed";
    this.speedLabel.style.opacity = "0.9";

    this.speedSlider = document.createElement("input");
    this.speedSlider.type = "range";
    this.speedSlider.min = "20";
    this.speedSlider.max = "300";
    this.speedSlider.step = "5";
    this.speedSlider.value = String(this.ui?.msPerImage ?? 100);
    this.speedSlider.style.width = "420px";

    this.speedValue = document.createElement("div");
    this.speedValue.textContent = `${this.ui?.msPerImage ?? 100}ms`;
    this.speedValue.style.minWidth = "84px";
    this.speedValue.style.textAlign = "right";
    this.speedValue.style.opacity = "0.85";

    this.speedRow.appendChild(this.speedLabel);
    this.speedRow.appendChild(this.speedSlider);
    this.speedRow.appendChild(this.speedValue);
    ...
    this.sightsPanel.appendChild(this.speedRow);
...
if (mobilePortrait) {
  ...
  this.banner.style.top = "6px";
  this.bannerImg.style.height = "58px";
  this.bannerText.style.fontSize = "30px";

  this.homePanel.style.width = "min(720px, calc(100vw - 220px))";
  this.homePanel.style.gridTemplateColumns = "repeat(3, 1fr)";
  this.homePanel.style.rowGap = "12px";
  this.homePanel.style.columnGap = "12px";
  this.homePanel.style.top = "52%";

  this.btnConnectLight.style.gridColumn = "1 / 4";
  this.btnConnectLight.style.height = "54px";
  this.btnConnectLight.style.fontSize = "20px";
  ...
  this.controls.style.bottom = "10px";
  this.controls.style.width = "min(720px, calc(100vw - 220px))";
  this.btnStart.style.height = "52px";
  this.btnStart.style.fontSize = "22px";
...
// (HTMLAudioElement volume is capped at 1.0).
  ...
  mount(parent) {
    ...
    // Image speed slider
    if (this.speedSlider) {
      // Ensure UI reflects current value on load
      const initMs = Number(this.ui?.msPerImage ?? 100);
      this.speedSlider.value = String(initMs);
      if (this.speedValue) this.speedValue.textContent = `${initMs}ms`;

      this.speedSlider.oninput = () => {
        const ms = Math.max(20, Math.min(500, parseInt(this.speedSlider.value, 10) || 100));
        this.ui.msPerImage = ms;
        if (this.speedValue) this.speedValue.textContent = `${ms}ms`;

        // Apply immediately: reset nextAt so the new tempo takes effect right away
        if (this.running) {
          this.ui._nextAt = performance.now() + ms;
        }
      };
    }
  ...
  _tick() {
    ...
    if (this.running) {
      ...
      if (!showEyesClosedScreen) {
        ...
        // catch up by advancing multiple times (capped) so tempo stays consistent.
        if (this.ui && this.ui.images && this.ui.images.length) {
          let steps = 0;

          // Initialize schedule on first run
          if (this.ui._nextAt === 0) {
            this.ui.updateSlideshow(nowMs);
          } else {
            // Advance based on the scheduled boundary, not just "now"
            while (this.ui._nextAt && nowMs >= this.ui._nextAt && steps < 6) {
              this.ui.updateSlideshow(this.ui._nextAt + 1);
              steps++;
            }
            // Update flash timers / overlay without forcing an extra advance
            this.ui.updateSlideshow(nowMs);
          }
        }
      }
